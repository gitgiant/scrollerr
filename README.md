...
Launches a throwaway Firefox profile with the extension loaded and reloads it on
every file save.

Or load it into your normal Firefox by hand: go to `about:debugging#/runtime/this-firefox`,
click **Load Temporary Add-on**, pick `src/manifest.json`.
...
Firefox disables extensions in private windows by default and no manifest key overrides
that. Enable it once per install: `about:addons` → Drip Scroller → **Allow in Private
Windows**.
...
The popup's **Scroll when** selector switches between `timer ends` and `clip ends`. In clip
